const totalHeightOfRows = (rowCount:number, rowHeight:number, marginBetweenRows:number) => 
...
const THREE_ROW_HEIGHT = totalHeightOfRows(3, ROW_HEIGHT, MARGIN_BETWEEN_ROWS);
const TWO_ROWS_HEIGHT = totalHeightOfRows(2, ROW_HEIGHT, MARGIN_BETWEEN_ROWS);

const scrollState = (height:number, count:number, isMobile:boolean=false) => {
  if(isMobile){
    return {
      paddingTop:0,
      paddingBottom:0,
  ...
  const shouldPadTop = scrollY > THREE_ROW_HEIGHT;
